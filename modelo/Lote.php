...
class Lote{
    ...
    function buscar(){
        if(!empty($_POST['consulta'])){
            ...
            JOIN presentacion on prod_present=id_presentacion AND producto.nombre like ':consulta' ORDER BY asc producto.nombre LIMIT 25";
            $query = $this->acceso->prepare($sql);
    ...
    function devolver($id_lote,$cantidad,$vencimiento,$producto,$proveedor){
        ...
        if(!empty($lote)){
            ...
            $sql="SELECT * FROM lote WHERE vencimiento=:vencimiento AND lote_id_prov=:producto AND lote_id_prov=:proveedor";
            $query = $this->acceso->prepare($sql);
            ...
            if(!empty($lote_nuevo)){
                ...
                $sql="INSERT INTO lote(id_lote,stock,vencimiento,lote_id_prod,lote_id_prov) VALUES(:id,:stock,:vencimiento,:producto,:proveedor)";
                $query = $this->acceso->prepare($sql);
