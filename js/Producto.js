$(document).ready(function(){
                            ...
                            <div class="row">
                                <div class="col-7">
                                    ...
                                    <ul class="ml-4 mb-0 fa-ul text-muted">
                                        ...
                                        <li class="small"><span class="fa-li"><i class="fas fa-lg fa-prescription-bottle-alt"></i></span> Adicional: ${producto.adicional}
                                        <li class="small"><span class="fa-li"><i class="fas fa-lg fa-flask"></i></span> Laboratorio: ${producto.laboratorio}
                                        <li class="small"><span class="fa-li"><i class="fas fa-lg fa-copyright"></i></span> Tipo: ${producto.tipo}
                                        <li class="small"><span class="fa-li"><i class="fas fa-lg fa-pills"></i></span> Presentacion: ${producto.presentacion}
                                    </ul>
                                </div>
    ...
    $(document).on('click','#button-reporte-pdf',(e)=>{
        Mostrar_loarder('generarReportePdf');
        funcion = 'reporte_productos_pdf';
        $.post('../controlador/ProductoController.php',{funcion},(response)=>{
            console.log(response);
            if(response==''){
                Cerrar_loader('exito_reporte');
                window.open('../pdf/prod-'+funcion+'.pdf','_blank'); //abrir pdf en pestaña nueva
            }
            else{
    ...
    $(document).on('click','#button-reporte-excel',(e)=>{
        Mostrar_loarder_Excel('generarReporteExcel');
        funcion = 'reporte_productos_excel';
        $.post('../controlador/ProductoController.php',{funcion},(response)=>{
            console.log(response);
            if(response==''){
                Cerrar_loader_Excel('exito_reporte');
                window.open('../excel/reporte_productos.xlsx','_blank'); //abrir pdf en pestaña nueva
            }
            else{
                Cerrar_loader_Excel('error_reporte');
            }
    ...
    function Mostrar_loarder(Mensaje){
        ...
        switch (Mensaje) {
            case 'generarReportePdf':
                texto = 'Adjuntado los datos, espere por favor unos segundos...';
                mostrar = true;
                break;
        }
        if(mostrar){
            Swal.fire({
                title: 'Generando reporte en PDF',
                text: texto,
    ...
    function Cerrar_loader(Mensaje){
        ...
        switch (Mensaje) {
            case 'exito_reporte':
                tipo ='success';
                texto = 'Reporte generado correctamente.';
                mostrar = true;
                break;

            case 'error_reporte':
                tipo ='error';
                texto = 'Error al generar reporte, intente de nuevo o comuniquese con el administrador del sistema.';
                mostrar = true;
                break;
    ...
    function Mostrar_loarder_Excel(Mensaje){
        var texto = null;
        var mostrar = false;
        switch (Mensaje) {
    ...
    function Cerrar_loader_Excel(Mensaje){
        var tipo = null;
        var texto = null;
        var mostrar = false;
        switch (Mensaje) {
